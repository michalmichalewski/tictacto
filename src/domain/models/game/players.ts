...
export class Players {
    ...
    private checkPlayerExists(player: Player): boolean {
        return this.players.findIndex(existingPlayer => {
            return existingPlayer.sign === player.sign;
        }) === -1;
    }
    ...
    add(player: Player) {
        ...
        if(!this.checkPlayerExists(player)) {
            throw new Error(`2#Cannot join the game. Player with ${player.sign} exists`);
        }

        this.players.push(player);
    }

    findBySign(sign: Sign): Player  {
        const player = this.players.find(player => player.sign === sign);
        if(player === undefined) {
